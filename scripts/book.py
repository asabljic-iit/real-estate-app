...
def get_renter_bookings(renter_id, agency=None):
    ...
    if agency is None:
        query = """
        SELECT B.*, street, city, state, credit_card_no FROM Booking B
        INNER JOIN Property P ON B.property_id = P.property_id 
        INNER JOIN CreditCard C ON B.credit_card_id = C.credit_card_id
        WHERE B.renter_id = %s 
    ...
    query = """
    SELECT B.*, email_address, name FROM Booking AS B 
    INNER JOIN Property AS P ON B.property_id = P.property_id 
    INNER JOIN Users AS U ON B.renter_id = U.user_id
    WHERE agency = %s ORDER BY start_date DESC
    """
    params = [agency]
    return execute_query(query, tuple(params), fetch_mode='all')
...
def cancel_booking(renter_id, booking_id, property_id, agency=None): 
    ...
    if agency is None:
        query = """
        DELETE FROM Booking 
        WHERE renter_id = %s AND booking_id = %s
        """
    ...
    query = """
    DELETE FROM Booking AS B USING Property AS P 
    WHERE B.property_id = P.property_id AND B.booking_id = %s 
    AND B.property_id = %s AND P.agency = %s
    """
...
def edit_booking(renter_id, booking_id, new_start_date, new_end_date, agency=None):
    ...
    query = """
    UPDATE Booking AS B 
    SET start_date = %s, end_date = %s FROM Property AS P 
    WHERE B.property_id = P.property_id AND B.booking_id = %s AND P.agency = %s
    """
